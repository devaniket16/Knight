...
const boardSize = 8;
const board = new Array(boardSize).fill().map(() => new Array(boardSize).fill(-1));
    ...
    const traverse = () => {
        ...
        for (const [dx, dy] of moves) {
            ...
            if (isValidMove(nextX, nextY)) {
                ...
                movesWithScores.push([nextX, nextY, score]);
            }
        }
        ...
        movesWithScores.sort((a, b) => a[2] - b[2]);

        for (const [nx, ny, _] of movesWithScores) {
            moveNumber++;
            board[nx][ny] = moveNumber;
            const square = chessboard.children[nx * boardSize + ny];
            square.classList.add('visited');
            square.textContent = moveNumber;
